...
class Cache(collectionsAbc.MutableMapping):
...
class _TimedCache(Cache):
    ...
    def __init__(self, maxsize, timer=time.time(), getsizeof=None):
        Cache.__init__(self, maxsize, getsizeof)
    ...
    def currsize(self):
        with self.__timer as time:
            self.expire(time)
            # ....
            return super(_TimedCache, self).currsize
...
class TTLCache(_TimedCache):
    ...
    def __getlink(self, key):
        ...
        tmp = collections.OrderedDict(
            [i for i in self.__links.items()] + [(key, value)]
        )
...
class TLRUCache(_TimedCache):
    ...
    def __init__(self, maxsize, ttu, timer=time.time(), getsizeof=None):
        _TimedCache.__init__(self, maxsize, timer, getsizeof)
